...
static bool
DuckdbCacheObject(Datum object) {
	auto db = DuckdbOpenDatabase();
	auto connection = duckdb::make_uniq<duckdb::Connection>(*db);
	auto &context = *connection->context;
	...
	auto cache_object_query = duckdb::StringUtil::Format("SELECT 1 FROM '%s';", object_path.c_str());
...
cache(PG_FUNCTION_ARGS) {
	...
	bool result = pgduckdb::DuckdbCacheObject(object);
	PG_RETURN_BOOL(result);
}
